...
def is_thank_you(landmarks):
    hand_center_x = (landmarks[mp_hands.HandLandmark.INDEX_FINGER_TIP].x + landmarks[mp_hands.HandLandmark.PINKY_TIP].x) / 2
    hand_center_y = (landmarks[mp_hands.HandLandmark.INDEX_FINGER_TIP].y + landmarks[mp_hands.HandLandmark.PINKY_TIP].y) / 2
    if landmarks[mp_hands.HandLandmark.WRIST].x < hand_center_x and landmarks[mp_hands.HandLandmark.WRIST].y < hand_center_y:
        return True
    return False
...
def is_okay(landmarks):
    ...
    position=get_hand_position(landmarks)
    if thumb_tip.y > index_tip.y and position==1 and middle_finger_tip.y > ring_finger_tip.y and all(abs(x - promedio_x) <= variacion_max for x in x_coords):
        return True
    return False
